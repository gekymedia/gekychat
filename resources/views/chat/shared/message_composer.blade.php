...
<footer class="message-input-container border-top bg-card position-sticky bottom-0" role="form" 
        aria-label="Send message{{ $isGroup ? ' to group' : '' }}"
  ...
  <div class="container-fluid px-0">
    ...
    <form id="chat-form" action="{{ $action }}" method="POST" 
      ...
      <div class="input-group composer align-items-end" id="drop-zone" role="group" aria-label="Message composer">
        {{-- Emoji Button --}}
        <button class="btn btn-ghost flex-shrink-0" type="button" id="emoji-btn" 
                aria-label="Add emoji" title="Add emoji">
          <i class="bi bi-emoji-smile" aria-hidden="true"></i>
        </button>

        {{-- Textarea for multi-line input --}}
        <div class="form-control-wrapper flex-grow-1 position-relative" style="min-width: 0;">
          <textarea name="body" class="form-control message-input flex-grow-1" 
                 placeholder="{{ $placeholder }}" id="message-input" 
        ...
        <div class="btn-group flex-shrink-0" role="group" aria-label="Message actions">
          ...
          @if(!$isGroup)
            <button class="btn btn-ghost" type="button" id="security-btn" 
                    aria-label="Security options" title="Security options">
            ...
            <li role="none">
              <label class="dropdown-item d-flex align-items-center gap-2 cursor-pointer" role="menuitem">
                ...
                <input type="file" name="attachments[]" class="d-none" 
                       id="doc-upload" multiple accept=".pdf,.doc,.docx,.txt,.zip,.rar"
        ...
        {{-- Send Button --}}
        <button class="btn btn-wa flex-shrink-0" type="submit" id="send-btn" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (emojiButton) {
      emojiButton.addEventListener('click', function(e) {
        ...
        if (window.emojiPicker && typeof window.emojiPicker.toggle === 'function') {
          ...
          console.warn('Emoji picker not available - make sure emoji_picker.blade.php is included');
        }
      });
    }
  }
  ...
  function updateAttachmentPreview() {
    ...
    // Update count
    attachmentCount.textContent = `${selectedFiles.length} file${selectedFiles.length !== 1 ? 's' : ''}`;
  ...
  async function handleFormSubmit(e) {
    ...
    try {
      ...
      // Set reply_to_id from the reply preview if it exists
